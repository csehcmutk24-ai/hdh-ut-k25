...
int inc_vma_limit(struct pcb_t *caller, int vmaid, addr_t inc_sz)
{
...
#ifdef MM64
  inc_amt = (inc_sz + PAGING64_PAGESZ - 1) & ~(PAGING64_PAGESZ - 1);
  incnumpage = inc_amt / PAGING64_PAGESZ;
#else
  inc_amt = (inc_sz + PAGING_PAGESZ - 1) & ~(PAGING_PAGESZ - 1);
  ...
  /* Cập nhật lại giới hạn vùng nhớ (sbrk và vm_end) */
